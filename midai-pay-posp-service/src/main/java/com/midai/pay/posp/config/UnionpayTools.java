...
public class UnionpayTools {
	...
	if (length > old) {
	    ...
	    if (x.length > length)
		throw new IllegalArgumentException((new StringBuilder("Numeric value is larger than intended length: "))
			.append(s).append(" LEN ").append(length).toString());
     ...
     *             （MAB) b,对MAB，按每8个字节做异或（不管信息中的字符格式），如果最后不满8个字节，则添加"0X00" c,
     *             将异或运算后的最后8个字节（RESULT BLOCK）转换成16 个HEXDECIMAL： d,取前8 个字节用MAK加密
     *             e,将加密后的结果与后8 个字节异或 f,用异或的结果TEMP BLOCK 再进行一次双倍长密钥算法运算。
     *             g,将运算后的结果（ENC BLOCK2）转换成16 个HEXDECIMAL： h,取前8个字节作为MAC值。
     */
	...
	byte[] desEnTmp = EncryptUtils.encrypt2(desSrc, mackey);
	byte[] desEnTmp2 = new byte[8];
	System.arraycopy(desEnTmp, 0, desEnTmp2, 0, 8);

	desEnTmp = bytesXOR(desEnTmp2, desSrc2);
	System.out.println(ByteUtil.convertByteArrayToHexStr(desEnTmp) + "--------");
	logger.debug(ByteUtil.convertByteArrayToHexStr(desEnTmp) + "----xor");
	byte[] desEn = EncryptUtils.encrypt2(desEnTmp, mackey);
    ...
    public static String calcMacXor(byte[] input) throws Exception {
     ...
     * @throws Exception
     */
    public static String encryptMac(String key, byte[] input) throws Exception {
	// 对MAB，按每8个字节做异或（不管信息中的字符格式），如果最后不满8个字节，则添加"0X00"
	byte[] macKey = ByteUtil.hexStringToByteArray(key);
	int length = input.length;
	int x = length % 8;
	...
	// 拆分异或结果
	xorInit = ByteUtil.convertByteArrayToHexStr(xorInit).getBytes();
	byte[] a = new byte[8];
	byte[] b = new byte[8];
	System.arraycopy(xorInit, 0, a, 0, a.length);
	System.arraycopy(xorInit, 8, b, 0, b.length);
	// 加密前8字节
	byte[] result = EncryptUtils.encrypt1(a, macKey);
	// 加密结果与后8字节异或
	for (int j = 0; j < 8; j++) {
	    result[j] ^= b[j];
	}
	// 对异或结果再做一次加密
	result = EncryptUtils.encrypt1(result, macKey);
	String mac = ByteUtil.convertByteArrayToHexStr(result);
	System.out.println("mac++++" + mac);
	// 得到的数据前8字节作为mac结果
	String macZ = mac.substring(0, 8);
	return macZ;
